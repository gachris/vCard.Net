...
{
    /// <summary>
    /// The vCard equivalent of the .NET <see cref="DateTime"/> class.
    /// <remarks>
    /// In addition to the features of the <see cref="DateTime"/> class, the <see cref="CardDateTime"/>
    /// class handles time zone differences, and integrates seamlessly into the vCard framework.
    /// </remarks>
    /// </summary>
    public sealed class CardDateTime : EncodableDataType, IDateTime
    {
        public static CardDateTime Now => new CardDateTime(DateTime.Now);

        public static CardDateTime Today => new CardDateTime(DateTime.Today);
        ...
        public CardDateTime(IDateTime value) => Initialize(value.Value, value.TzId);

        public CardDateTime(DateTime value) : this(value, null) { }
        ...
        /// </summary>
        public CardDateTime(DateTime value, string tzId) => Initialize(value, tzId);

        public CardDateTime(int year, int month, int day, int hour, int minute, int second)
        {
            Initialize(year, month, day, hour, minute, second, null);
            HasTime = true;
        }

        public CardDateTime(int year, int month, int day, int hour, int minute, int second, string tzId)
        {
            Initialize(year, month, day, hour, minute, second, tzId);
            HasTime = true;
        }

        public CardDateTime(int year, int month, int day) : this(year, month, day, 0, 0, 0) { }
        public CardDateTime(int year, int month, int day, string tzId) : this(year, month, day, 0, 0, 0, tzId) { }

        public CardDateTime(string value)
        {
            var serializer = new DateTimeSerializer();
            CopyFrom(serializer.Deserialize(new StringReader(value)) as ICopyable);
        }

        private void Initialize(int year, int month, int day, int hour, int minute, int second, string tzId) => Initialize(CoerceDateTime(year, month, day, hour, minute, second, DateTimeKind.Local), tzId);
        ...
        public override ICardObject AssociatedObject
        {
        ...
        public bool Equals(CardDateTime other)
            => this == other;

        public override bool Equals(object other)
            => other is IDateTime && (CardDateTime)other == this;
        ...
        public static bool operator <(CardDateTime left, IDateTime right)
        ...
        public static bool operator ==(CardDateTime left, IDateTime right) => ReferenceEquals(left, null) || ReferenceEquals(right, null)
                ? ReferenceEquals(left, right)
                : right is CardDateTime
        ...
        public static TimeSpan operator -(CardDateTime left, IDateTime right)
        {
            left.AssociateWith(right);
            return left.AsUtc - right.AsUtc;
        }

        public static IDateTime operator -(CardDateTime left, TimeSpan right)
        {
        ...
        public static implicit operator CardDateTime(DateTime left) => new CardDateTime(left);
        ...
        public IDateTime ToTimeZone(string tzId)
        {
            ...
            return converted.Zone == DateTimeZone.Utc
                ? new CardDateTime(converted.ToDateTimeUtc(), tzId)
                : new CardDateTime(DateTime.SpecifyKind(converted.ToDateTimeUnspecified(), DateTimeKind.Local), tzId);
        }
