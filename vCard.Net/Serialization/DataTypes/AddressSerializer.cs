...
public class AddressSerializer : EncodableDataTypeSerializer
{
    ...
    public override string SerializeToString(object obj)
    {
        ...
        {
            num = 1;
            array[0] = version == vCardVersion.vCard21 ? address.POBox.RestrictedEscape() : address.POBox.Escape();
        }

        if (address.ExtendedAddress != null && address.ExtendedAddress.Length > 0)
        {
            num = 2;
            array[1] = version == vCardVersion.vCard21 ? address.ExtendedAddress.RestrictedEscape() : address.ExtendedAddress.Escape();
        }

        if (address.StreetAddress != null && address.StreetAddress.Length > 0)
        {
            num = 3;
            array[2] = version == vCardVersion.vCard21 ? address.StreetAddress.RestrictedEscape() : address.StreetAddress.Escape();
        }

        if (address.Locality != null && address.Locality.Length > 0)
        {
            num = 4;
            array[3] = version == vCardVersion.vCard21 ? address.Locality.RestrictedEscape() : address.Locality.Escape();
        }

        if (address.Region != null && address.Region.Length > 0)
        {
            num = 5;
            array[4] = version == vCardVersion.vCard21 ? address.Region.RestrictedEscape() : address.Region.Escape();
        }

        if (address.PostalCode != null && address.PostalCode.Length > 0)
        {
            num = 6;
            array[5] = version == vCardVersion.vCard21 ? address.PostalCode.RestrictedEscape() : address.PostalCode.Escape();
        }

        if (address.Country != null && address.Country.Length > 0)
        {
            num = 7;
            array[6] = version == vCardVersion.vCard21 ? address.Country.RestrictedEscape() : address.Country.Escape();
        }
