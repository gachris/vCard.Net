...
namespace vCard.Net.Serialization
{
    public class SerializerFactory : ISerializerFactory
    {
        ...
        public virtual ISerializer Build(Type objectType, SerializationContext ctx)
        {
            ...
            if (typeof(ICardComponent).IsAssignableFrom(objectType))
            {
                s = new ComponentSerializer(ctx);
            }
            else if (typeof(ICardProperty).IsAssignableFrom(objectType))
            {
                s = new PropertySerializer(ctx);
            }
            else if (typeof(CardParameter).IsAssignableFrom(objectType))
            {
            ...
            else if (typeof(ICardDataType).IsAssignableFrom(objectType))
            {
